...
A lightweight Python network reconnaissance tool. PySec scans a target domain for open ports **concurrently**, grabs service banners, analyzes HTTP responses, and saves a structured JSON report of the results — built as a hands-on project to learn Python, core networking/security concepts, and concurrency.
...
Note: because ports are scanned concurrently, output for different ports may appear interleaved rather than in the order they were entered — this is expected and is a sign the threads are running in parallel.
...
Built as a learning project to practice Python fundamentals (functions, error handling, file I/O) alongside core networking, cybersecurity, and concurrency concepts (sockets, ports, protocols, banner grabbing, multithreading, thread safety).